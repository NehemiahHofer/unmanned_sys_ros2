...
class OdomLocNode(Node):
    def __init__(self, sub_topic, vel_topic):
...
def main():
    ...
    myData = ["time", "x", "y", "yaw", "linear_velocity", "angular_velocity"]

    # this creates a filename which contains the current date/time RaspberryPi does not have a real-time clock, the files
    # will have the correct sequence (newest to oldest is preserved) but unless you set it explicitly the time will not
    ...
    while rclpy.ok():
        ...
        myData = [now, odom_node.current_position[0], odom_node.current_position[1],
                  odom_node.orientation_euler[2], odom_node.linear_velocity, odom_node.angular_velocity]
